...
class GroupsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        if self.mode == "search" {
            ...
            if (self.lng != nil && self.lat != nil) {
                searchUrl = "http://qifanw2-env.us-west-2.elasticbeanstalk.com/index.php?keyword=\(self.keyword!)&type=group&lat=\(self.lat!)&lng=\(self.lng!)&limit=10"
            }else{
                searchUrl = "http://qifanw2-env.us-west-2.elasticbeanstalk.com/index.php?keyword=\(self.keyword!)&type=group&limit=10"
            }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if self.mode == "search"{
            ...
            return (UserDefaults.standard.array(forKey: "group")?.count)!
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var tmparray = [String]()
        var tmpid: String?
        tmparray = UserDefaults.standard.array(forKey: "group") as! [String]
        ...
        if self.mode == "search"{
            ...
            if(UserDefaults.standard.array(forKey: "group")?.count != 0){
                var tmpid: String?
                var tmpname: String?
                var tmpurl: String?
                var imgUrl: URL?
                tmpid = UserDefaults.standard.array(forKey: "group")?[indexPath.row] as? String
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.type = "group"
        self.performSegue(withIdentifier: "groupDetail", sender: nil)
    }
